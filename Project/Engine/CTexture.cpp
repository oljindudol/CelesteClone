...
{
	m_Desc = _vecTex[0].Get()->m_Desc; //_vecTex[0].Get()->m_Tex2D->GetDesc(&_vecTex[0].Get()->m_Desc);
	...
	for (int i = 0; i < _vecTex.size(); ++i)
	{
		...
		CONTEXT->UpdateSubresource(m_Tex2D.Get(), iOffset, nullptr, _vecTex[i].Get()->GetPixels() //_vecTex[i]->GetSysMem()
			, _vecTex[i].Get()->GetRowPitch(), _vecTex[i].Get()->GetSlicePitch());
	}
