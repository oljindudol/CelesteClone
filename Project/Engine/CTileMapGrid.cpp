...
void CTileMapGrid::Init()
{
	...
	for (int y = 0; y < iRow + 1; ++y) {
		Vector3 vStartPos = Vector3(vOriginLBPos.x, vOriginLBPos.y + y * vGridSize.y, -.1f);// vObjPos.z);
		Vector3 vEndPos = Vector3(vOriginRTPos.x, vOriginLBPos.y + y * vGridSize.y, -.1f); //vObjPos.z );


		AddBuffer(vStartPos / vScale, vEndPos / vScale); // 1 미만 사이즈로 만들어줌.
	}
	for (int x = 0; x < iCol + 1; ++x) {
		Vector3 vStartPos = Vector3(vOriginLBPos.x + x * vGridSize.x, vOriginLBPos.y, -.1f); //vObjPos.z );
		Vector3 vEndPos = Vector3(vOriginLBPos.x + x * vGridSize.x, vOriginRTPos.y, -.1f); //vObjPos.z );
		AddBuffer(vStartPos / vScale, vEndPos / vScale); // 1 미만 사이즈로 만들어줌.
	}
}
